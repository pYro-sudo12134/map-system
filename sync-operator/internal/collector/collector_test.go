...
func TestCollect_Success(t *testing.T) {
	...
	col := NewCollector(Config{QueueName: "SQS_SYNC", CheckInterval: time.Hour}, reader, exporter)
	...
	if got := exporter.lengths["SQS_SYNC"]; got != 42 {
		t.Fatalf("SetQueueLength: expected 42, got %d", got)
	}
	if got := exporter.timestamps["SQS_SYNC"]; got == 0 {
		t.Fatal("SetLastScrapeTimestamp: expected non-zero timestamp")
	}
	if got := exporter.errorCounts["SQS_SYNC"]; got != 0 {
...
func TestCollect_ReaderError(t *testing.T) {
	...
	if got := exporter.errorCounts["SQS_SYNC"]; got != 1 {
		t.Fatalf("IncScrapeErrors: expected 1, got %d", got)
	}
	if _, exists := exporter.lengths["SQS_SYNC"]; exists {
		t.Fatal("SetQueueLength should not be called on error")
	}
	if _, exists := exporter.timestamps["SQS_SYNC"]; exists {
...
func TestStart_TickerPolling(t *testing.T) {
	...
	exporter := newFakeExporter()
	col := NewCollector(Config{QueueName: "SQS_SYNC", CheckInterval: 5 * time.Millisecond}, reader, exporter)
